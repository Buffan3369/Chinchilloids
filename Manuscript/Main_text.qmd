...
Became larger as time went forward (DTT plot), by-product of the Bergman's rule? (*appropriate refs*)
...
The massive diversity of the Late Miocene Ituzaingó formation [@candela2005] likely drives the reconstructed diversity peak that we infer.
...
#| out-height: 7cm
#| out-width: 14cm
#| fig-cap: Comparative occurrence-based chinchilloid diversity reconstructions. Estimates were either produced in a bayesian preservation-aware framework jointly assessing the times of speciation and extinction of each species from the temporal occurrence database (PyRate) **(A)**, or using a simulation-based deep-learning framework incorporating a wide variety of biases to reconstruct the temporal variations of the "true" (*i*.*e*., corrected) diversity (DeepDive) **(B)**. In **(A)**, the full line represents the median diversity estimate across all PyRate runs that achieved convergence and the ribbon shows its associated 95% HPD interval. In **(B)**, the full line shows the mean diversity estimate per 1Myr-bin obtained across the 100 replicates of empirical prediction while the darker and lighter ribbons show the intervals respectively encompassing 50% and 95% of the per-bin diversity distribution across the 100 replicates.

knitr::include_graphics("../Figures/Main/Diversity_through_time.png")
```
