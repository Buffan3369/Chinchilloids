...
for(lvl in c("genus", "species")){
  for(ana in c("1-Full", "2-Singleton", "3-NoCaribbea", "3-Spatially_scaled")){
    ...
    # LTT table
    path_to_ltt <- list.files(paste0("./Results/RJMCMC/", lvl, "/", ana, "/LTT/"), pattern = "se_est_ltt.txt", full.names = TRUE)[1]
    ...
    # q-rates table
    path_to_q <- paste0("./Results/RJMCMC/", lvl, "/", ana, "/Q_SHIFTS/Parsed_Q_rates.txt")
