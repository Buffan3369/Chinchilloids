...
for(i in 1:10){
  ### Body Mass ###
  # Impute missing data: randomly sample within c(3,4,5)
  BM$`BodyMass category`[na_BM] <- 
    extraDistr::rdunif(n = length(na_BM), min = 3, max = 5)
  # Save the median
  ...
  ### Hypsodonty Index
  # Impute missing data: sample in a Bernoulli with p=0.4 (either HI = 3 or HI = 4), introducing a light bias towards euhypsodonty
  #   as we believe it to be more likely given our data
