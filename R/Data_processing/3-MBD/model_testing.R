...
post_MBD1 <- read.table("./Results/MBD/Old/unphased/MBD_scaled_unphased_hsp0/combined_20_KEEP.log", header = T)
...
post_MBD2 <- read.table("./Results/MBD/Old/species/combined_20_KEEP.log", header = T)
lkl_MBD2 <- post_MBD2$likelihood
marg_lkl2 <- harmonic.mean(lkl_MBD2)

post_MBD3 <- read.table("./Results/MBD/Old/unphased/MBD_scaled_unphased/combined_17_KEEP.log", header = T)
lkl_MBD3 <- post_MBD3$likelihood
marg_lkl3 <- harmonic.mean(lkl_MBD3)

post_MBD4 <- read.table("./Results/MBD/Old/species_rmDD/combined_20_KEEP.log", header = T)
